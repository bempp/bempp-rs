...
mod assembly_internal {
    use crate::assembly::batched::*;
    // TODO: use T not f64
    pub trait BatchedAssemblerInternal: Sync {
        //! Internal functions for batched assembly
        //!
        //! These functions are not intended to be called directly by a user
        ...
        fn assemble_batch_singular<'a>(
            ...
            let mut k = rlst_dynamic_array2!(f64, [Self::DERIV_SIZE, npts]);
        ...
        fn assemble_batch_nonadjacent<'a, const NPTS_TEST: usize, const NPTS_TRIAL: usize>(
            ...
            for (trial_cell_i, trial_cell) in trial_cells.iter().enumerate() {
                ...
                trial_evaluator
                    .compute_points(trial_cell_gindex, &mut trial_mapped_pts[trial_cell_i]);
            }
...
pub trait BatchedAssembler: assembly_internal::BatchedAssemblerInternal + Sync {
    //! Batched assembler
    //!
...
impl Default for LaplaceSingleLayerAssembler {
    fn default() -> Self {
        Self {
...
impl assembly_internal::BatchedAssemblerInternal for LaplaceSingleLayerAssembler {
...
impl Default for LaplaceDoubleLayerAssembler {
    fn default() -> Self {
        Self {
            kernel: Laplace3dKernel::<f64>::new(),
        }
...
impl assembly_internal::BatchedAssemblerInternal for LaplaceDoubleLayerAssembler {
...
impl Default for LaplaceAdjointDoubleLayerAssembler {
    fn default() -> Self {
        Self {
            kernel: Laplace3dKernel::<f64>::new(),
        }
...
impl assembly_internal::BatchedAssemblerInternal for LaplaceAdjointDoubleLayerAssembler {
    const DERIV_SIZE: usize = 4;
    unsafe fn singular_kernel_value(
        &self,
        k: &Array<f64, BaseArray<f64, VectorContainer<f64>, 2>, 2>,
