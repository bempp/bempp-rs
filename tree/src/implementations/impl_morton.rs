...
impl MortonKey {
    ...
    /// Check if two keys are adjacent with respect to each other when they are known to be on the same level
...
mod test {
    ...
    use rlst::{dense::{RawAccess, Shape, rlst_pointer_mat, LayoutType, Dynamic}, common::tools::PrettyPrint};
    use std::vec;
