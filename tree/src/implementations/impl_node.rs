use solvers_traits::fmm::{FmmNodeData, FmmLeafNodeData};
...
impl FmmNodeData for Node {
    ...
    fn set_order(&mut self, order: usize) {
        ...
        self.data.displacement = self.data.field_size
            .iter()
            .scan(0, |state, &x| {
                let tmp = *state;
                *state += x;
    ...
    fn set_multipole_expansion(&mut self, data: &Self::CoefficientData) {
        for (i, elem) in self.data.raw[self.data.displacement[1]..].iter_mut().enumerate() {
            *elem = data[i]
        }
    }
}
