...
impl<T, U> FmmLoop for FmmDataLinearSparse<T, U>
where
    T: Fmm,
    U: Scalar<Real = U> + Float + Default,
    FmmDataLinearSparse<T, U>: SourceTranslation + FieldTranslation<U> + TargetTranslation,
{
...
mod test {
    ...
    fn test_fmm_fft_f64_sparse() {
        ...
        let datatree = FmmDataLinearSparse::new(fmm, &charge_dict).unwrap();
