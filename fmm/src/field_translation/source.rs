...
impl<T, U, V> SourceTranslation for FmmData<KiFmmLinear<SingleNodeTree<V>, T, U, V>, V>
where
...
impl<T, U, V> SourceTranslation for FmmDataSparse<KiFmmLinear<SingleNodeTree<V>, T, U, V>, V>
where
    T: Kernel<T = V> + ScaleInvariantKernel<T = V> + std::marker::Send + std::marker::Sync,
    U: FieldTranslationData<T> + std::marker::Sync + std::marker::Send,
    V: Scalar<Real = V> + Float + Default + std::marker::Sync + std::marker::Send,
...
mod test {
    ...
    #[test]
    fn test_upward_pass_sphere_sparse() {
        let npoints = 10000;
        let points = points_fixture_sphere(npoints);
        let global_idxs = (0..npoints).collect_vec();
        let charges = vec![1.0; npoints];
