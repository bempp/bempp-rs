...
use crate::types::{FmmData, FmmDataSparse, KiFmmLinear, SendPtrMut, FmmDataAdaptive};
...
impl<T, U>
    FmmData<KiFmmLinear<SingleNodeTree<U>, T, FftFieldTranslationKiFmm<U, T>, U>, U>
where
    T: Kernel<T = U>
...
impl<T, U>
    FmmDataSparse<KiFmmLinear<SingleNodeTree<U>, T, FftFieldTranslationKiFmm<U, T>, U>, U>
where
    T: Kernel<T = U>
        + ScaleInvariantKernel<T = U>
        + std::marker::Send
...
impl<T, U>
    FmmDataAdaptive<KiFmmLinear<SingleNodeTree<U>, T, FftFieldTranslationKiFmm<U, T>, U>, U>
where
    T: Kernel<T = U>
        + ScaleInvariantKernel<T = U>
        + std::marker::Send
...
impl<T, U> FieldTranslation<U>
    for FmmData<KiFmmLinear<SingleNodeTree<U>, T, FftFieldTranslationKiFmm<U, T>, U>, U>
where
    T: Kernel<T = U>
        + ScaleInvariantKernel<T = U>
        + std::marker::Send
...
impl<T, U> FieldTranslation<U>
    for FmmDataSparse<KiFmmLinear<SingleNodeTree<U>, T, FftFieldTranslationKiFmm<U, T>, U>, U>
where
    T: Kernel<T = U>
        + ScaleInvariantKernel<T = U>
        + std::marker::Send
...
impl<T, U> FieldTranslation<U>
    for FmmData<KiFmmLinear<SingleNodeTree<U>, T, SvdFieldTranslationKiFmm<U, T>, U>, U>
where
    T: Kernel<T = U>
        + ScaleInvariantKernel<T = U>
        + std::marker::Send
    ...
    fn m2l<'a>(&self, level: u64) {
        ...
        for (j, source) in sources.iter().enumerate() {
            let v_list = source
                ...
                .iter()
                .flat_map(|pn| pn.children())
                .filter(|pnc| !source.is_adjacent(pnc) && self.fmm.tree().get_all_keys_set().contains(pnc))
                .collect_vec();
...
mod test {
    ...
    use bempp_tree::{implementations::helpers::points_fixture_sphere, types::single_node::SingleNodeTree};
    use float_cmp::assert_approx_eq;

    use crate::{charge::build_charge_dict, types::{KiFmmLinear, FmmDataAdaptive, FmmDataSparse, FmmData}};

    use itertools::Itertools;
    use rlst::dense::{RawAccess, rlst_pointer_mat};
    use bempp_traits::{tree::Tree, fmm::{FmmLoop, InteractionLists, Fmm}, kernel::Kernel};
    use bempp_traits::field::FieldTranslationData;

    use rlst::{
        common::traits::*,
        dense::{traits::*, Dot, MultiplyAdd, VectorContainer, rlst_dynamic_mat, rlst_col_vec},
    };    
    ...
    pub fn test_field_translation_adaptive() {
        ...
        let datatree = FmmDataSparse::new(fmm, &charge_dict).unwrap();

        datatree.run(false);

        println!("N AT LEVEL 2 {:?}", datatree.fmm.tree.get_keys(3).unwrap().iter().len());
        for leaf_node in datatree.fmm.tree.get_keys(4).unwrap().iter() {
            if let Some(v_list) = datatree.fmm.get_v_list(leaf_node) {
                ...
                let &level_index = datatree.level_index_pointer[leaf_level as usize].get(leaf_node).unwrap();
                let leaf_local = datatree.level_locals[leaf_level as usize][level_index];
                let mut leaf_local = unsafe { std::slice::from_raw_parts_mut(leaf_local.raw, ncoeffs) };
                ...
                for source in v_list.iter() {
                    ...
                    let &level_index = datatree.level_index_pointer[source_level as usize].get(source).unwrap();
                    let source_multipole = datatree.level_multipoles[source_level as usize][level_index];
                    let source_multipole = unsafe { std::slice::from_raw_parts(source_multipole.raw, ncoeffs)};
                ...
                let &level_index = datatree.level_index_pointer[parent.level() as usize].get(&parent).unwrap();
                let parent_local = datatree.level_locals[parent.level() as usize][level_index];
                let parent_local = unsafe { rlst_pointer_mat!['_, f64, parent_local.raw, (ncoeffs, 1), (1, ncoeffs)] };
                
                let upward_equivalent_surface = leaf_node.compute_surface(
                    datatree.fmm.tree.get_domain(),
                    datatree.fmm.order, 
                    datatree.fmm.alpha_inner
                );
                ...
                // println!("tmp {:?}", parent_local.data());
                // direct.iter_mut().zip(tmp.data()).for_each(|(d, t)| *d += t);
