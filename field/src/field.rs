...
use crate::types::{
    FftFieldTranslationKiFmmNew, SvdFieldTranslationKiFmm, SvdSourceToTargetOperatorData,
};
use crate::{
    array::flip3,
    fft::Fft,
    transfer_vector::compute_transfer_vectors,
    types::{FftFieldTranslationKiFmm, FftM2lOperatorData},
};
    ...
    Array<T, BaseArray<T, VectorContainer<T>, 2>, 2>: MatrixSvd<Item = T>,
{
    ...
    pub fn new(
        ...
        result.set_operator_data(order, domain);
...
impl<T, U> SourceToTargetData<U> for FftFieldTranslationKiFmmNew<T, U>
where
    T: Scalar<Real = T> + Float + Default + Fft,
    Complex<T>: Scalar,
    U: HomogenousKernel<T = T> + Default,
...
impl<T, U> FftFieldTranslationKiFmmNew<T, U>
where
    T: Float + Scalar<Real = T> + Default + Fft,
    Complex<T>: Scalar,
    U: HomogenousKernel<T = T> + Default,
{
    ...
    /// * `alpha` - The multiplier being used to modify the diameter of the surface grid uniformly along each coordinate axis.
    pub fn new(kernel: U, order: usize, domain: Domain<T>, alpha: T) -> Self {
        let mut result = FftFieldTranslationKiFmmNew {
            alpha,
            kernel,
            surf_to_conv_map: Vec::default(),
            conv_to_surf_map: Vec::default(),
        ...
        // Create maps between surface and convolution grids
        let (surf_to_conv, conv_to_surf) =
            FftFieldTranslationKiFmmNew::<T, U>::compute_surf_to_conv_map(order);
