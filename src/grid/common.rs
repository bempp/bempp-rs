...
// use num::Float;
// use rlst::{rlst_array_from_slice3, rlst_static_array, RlstScalar};
...
//     }
// }
// /// Compute integration elements from Jacobians.
// ///
// /// The Jacobian is assumed to be a [3, 2, npts] array in column-major order.
// pub(crate) fn compute_integration_element<T: RlstScalar<Real = T>>(jacobian: &[T], jdets: &mut [T])
// where
//     T::Real: num::Float,
// {
//     let npts = jdets.len();
//     let jacobian = rlst_array_from_slice3!(jacobian, [3, 2, npts]);
//     for (index, det) in jdets.iter_mut().enumerate() {
//         let jac = jacobian.view().slice(2, index);
//         let mut c = rlst_static_array!(T, 3);
//         jac.view()
//             .slice(1, 0)
//             .cross(jac.view().slice(1, 1), c.view_mut());
//         *det = c.norm_2();
//     }
// }
